...
r = session.get('http://americanarchivist.org/loi/AARC')
print (r.status_code)
html = r.text
...
csvHead = ["year", "volume", "issue", "type", "authors", "title", "doi", "url", "path"]
			...
			if int(year) == 2014:
				...
				for issue in issueList.findChildren():
					if "Issue" in issue.text:
						if issue.name == "a":
							issuePath = os.path.join(yearFolder, issue.text.replace("/", "-"))
							
							time.sleep(2)
							issuePage = session.get(issue['href'])
							print (issuePage.status_code)
							
							issueHtml = issuePage.text
							issueSoup = BeautifulSoup(issueHtml, "html.parser")
							articles = issueSoup.find_all('table', {'class': 'articleEntry'})
							for article in articles:
								time.sleep(2)				
								typeHeading = article.findPrevious('h2', {'class': 'tocHeading'})
								if int(year) > 2014:
									type = typeHeading.find("span").text.strip().title()
								else:
									type = typeHeading.find("div").text.strip().title()
								
								articleRow = [year, volume, issue.text, type]
								
								typeDir = os.path.join(issuePath, type.replace(":", "_")).replace(" ", "_")
								title = article.find('div', {'class': 'art_title'}).text
								if not "editorial policy" in title.lower():
									...
									url = "http://www.americanarchivist.org" + article.find('a', {'title': 'Opens new window'})["href"]
									doi = url.split("/pdf/")[1]
									articleRow.append(doi)
									articleRow.append(url)
									path = os.path.join(typeDir, doi.split("/")[1] + ".pdf")
									articleRow.append(path)
									if not os.path.isdir(typeDir):
										os.makedirs(typeDir)
										
									#download pdf
									print ("	Downloading " + str(title.encode("utf8")) + "...")
									pdfResponse = session.get(url)
									with open(path, 'wb') as pdf:
										pdf.write(pdfResponse.content)
									
									outFile = open(csvFile, "a", newline='', encoding="utf8")
									writer = csv.writer(outFile, delimiter=",")
